  ...
  return <main className="min-h-screen bg-slate-50 px-4 py-10 sm:px-6 sm:py-16">
    <div className="mx-auto max-w-5xl">
      ...
      <form onSubmit={submit} className="space-y-6">
        ...
        <div className="flex flex-col items-start justify-between gap-5 rounded-3xl bg-slate-950 p-5 text-white sm:flex-row sm:items-center sm:p-7"><div><p className="font-black">Sau khi gửi hồ sơ</p><p className="mt-1 text-sm leading-6 text-slate-300">Tham gia nhóm Zalo hướng dẫn: <a className="font-bold text-cyan-300 underline" href="https://zalo.me/g/uxjmkq913" target="_blank" rel="noreferrer">zalo.me/g/uxjmkq913</a></p></div><button disabled={loading} className="h-12 w-full rounded-xl bg-[#79BCC2] px-7 font-black text-slate-950 transition hover:bg-white disabled:cursor-wait disabled:opacity-60 sm:w-auto">{loading ? 'Đang gửi hồ sơ...' : 'Gửi hồ sơ đăng ký'}</button></div>
      </form>
    </div>
  </main>;
}
