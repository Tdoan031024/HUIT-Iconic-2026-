...
import { translate, localizeTable } from '@/src/i18n';
...
export default function VoteModal({ candidate, onClose, onSuccess }: VoteModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[1105] flex items-center justify-center overflow-y-auto bg-[rgba(7,16,31,0.52)] p-3 backdrop-blur-md sm:p-5" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <div
      ...
      >
        <div className="relative flex h-[72px] items-center justify-center border-b border-slate-200/80 px-5 sm:px-7">
          ...
          <h2 className="text-center text-[24px] font-bold leading-none tracking-normal text-slate-950 sm:text-[28px]">
            {language === 'en' ? 'Free Daily Voting' : 'Bình chọn miễn phí'}
          </h2>
          ...
          {step === 'confirm' ? (
            <div className="space-y-2">
              ...
              <section className="space-y-2">
                  ...
                  <div className="min-w-0 flex-1 text-center sm:text-left pt-0.5">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5 justify-start text-[11px] sm:text-xs">
                      ...
                      <span className="inline-flex items-center rounded bg-blue-50 px-2 py-0.5 font-semibold text-blue-700">
                        {localizeTable(candidate.contestTable, language) || getContestTableLabel(candidate)}
                      </span>
                      {candidate.sector && (
                        <span className="inline-flex items-center rounded bg-emerald-50 px-2 py-0.5 font-semibold text-emerald-700">
                          {language === 'en' ? 'Sector' : 'Lĩnh vực'}: {candidate.sector}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="mt-1.5 space-y-1 border-t border-slate-100/70 pt-1.5 text-[13px] sm:text-[14px]">
                      {candidate.representativeSchool && (
                        <div className="flex items-start gap-1.5 text-left">
                          <span className="font-bold text-slate-500 shrink-0 w-[80px]">{language === 'en' ? 'Faculty:' : 'Đơn vị:'}</span>
                      ...
                      {(candidate.teamName || candidate.leaderName) && (
                        <div className="flex items-start gap-1.5 text-left">
                          <span className="font-bold text-slate-500 shrink-0 w-[80px]">{language === 'en' ? 'Team:' : 'Thực hiện:'}</span>
                          <span className="text-slate-800 font-medium leading-tight">
                            ...
                            {candidate.leaderName && ` (${language === 'en' ? 'Leader' : 'Trưởng nhóm'}: ${candidate.leaderName})`}
                          </span>
                        </div>
                      )}
                    </div>
                ...
                {(candidate.description || (candidate as any).descriptionEn) && (
                  <p className="text-[13px] leading-relaxed text-slate-500 text-justify">
                    <span className="font-bold text-slate-700">{language === 'en' ? 'Summary: ' : 'Mô tả thí sinh: '}</span>
                    <span>{localizedText(language, candidate.description, (candidate as any).descriptionEn)}</span>
                  </p>
                )}
              </section>
                ...
                <div className="grid gap-4 sm:grid-cols-[188px_minmax(0,1fr)] sm:items-center sm:gap-5">
                  ...
                  <div className="min-w-0">
                    <h4 className="text-center text-[20px] font-bold tracking-normal text-slate-950 sm:text-left">
                      {language === 'en' ? 'Remaining Votes Today' : 'Lượt còn lại hôm nay'}
                    </h4>

                    <p className="mt-1.5 text-center text-[14px] leading-6 text-slate-600 sm:text-left">
                      {language === 'en' ? 'Each verified user gets 2 free votes daily.' : 'Mỗi tài khoản được tặng 2 lượt bình chọn miễn phí mỗi ngày.'}
                    </p>

                    {turnstileSiteKey && currentUser ? (
                      <div className="mt-3 flex justify-center" ref={turnstileRef} aria-label="Security check" />
                    ) : null}
                    ...
                    <div className="mt-4">
                      {!currentUser ? (
                        ...
                        >
                          {language === 'en' ? 'Sign in to vote' : 'Đăng nhập để bình chọn'}
                        </button>
                      ) : (
                        <button
                        ...
                        >
                          ...
                          {isVoting ? (language === 'en' ? 'Processing...' : 'Đang xử lý...') : (language === 'en' ? 'Confirm Vote' : 'Xác nhận bình chọn')}
                        </button>
                      )}
                    </div>
                  </div>
